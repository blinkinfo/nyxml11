...
class MLStrategy(BaseStrategy):
    ...
    async def check_signal(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            slot_end_ms = (int(slot_ts) * 1000) if isinstance(slot_ts, (int, float)) else int(slot_ts.timestamp() * 1000)  # exclusive boundary — slot_ts is seconds from scheduler, so multiply by 1000
